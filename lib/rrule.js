        ...
        switch (key) {
            ...
            case'EXDATE':
                for (var j = 0; j < value.length; j++) {
                    strValues[j] = dateutil.timeToUntilString(value[j]);
                }
                value = strValues;
                break;
            default:
	...
	*/
    ...
    _iter: function(iterResult) {
        ...
        */
        ...
        var i, j, k, dm, div, mod, tmp, pos, dayset, start, end, fixday, exdatemask;
        ...
        while (true) {
            ...
            if (plb(exdate)) {
                exdatemask = ii.exdateset(year, month, day, exdate);
            }
    ...
    for (i = 0; i < attrs.length; i++) {
        ...
        switch (key) {
            ...
            case 'EXDATE':
                value = value.split(',');
                for (j = 0; j < value.length; j++) {
                    value[j] = dateutil.untilStringToDate(value[j]);
                }
                options.exdate = value;
                break;
            case 'BYEASTER':
...
Iterinfo.prototype.exdateset = function exdateset (year, month, day, exdate) {
    var exdatemask = repeat(null, this.yearlen + 7);
    var yearordinal = this.yearordinal;
    _.map(exdate, function loopExdate (exday) {
        var date = exday.getDate();
        var day = dateutil.toOrdinal(
            new Date(year, month - 1, date)) - yearordinal;
        exdatemask[day] = 1;
    });
    return exdatemask;
};
